...
class ImageRecognitionService:
    ...
    def _initialize_model(self):
        ...
        try:
            if self.task == RecognitionTask.OBJECT_RECOGNITION:
                ...
                model_name = self.config.IMAGE_MODEL_MAP.get(self.task.value)
                if not model_name:
                    # Use a default model if not specified in config
                    from ultralytics import YOLO
                    self.model = YOLO("yolov8n.pt")  # Default to YOLOv8n standard format
                    logger.info("Default YOLOv8n model loaded for object recognition")
                else:
                    # Use the specified model from config
                    from ultralytics import YOLO
                    self.model = YOLO(model_name)
                    logger.info(f"Model loaded for object recognition: {model_name}")
                ...
                if not model_name:
                    ...
                    logger.info(f"Using default model for {self.task.value}: {model_name}")
                ...
                try:
                    ...
                    self.model = pipeline("image-classification", model=model, feature_extractor=processor,
                                          device=self.device)
                ...
                if not model_name:
                    # Use a default model if not specified in config
                    model_name = "selimsef/dfdc_deepfake_resnet50"
                    logger.info(f"Using default model for {self.task.value}: {model_name}")
    ...
    def _process_object_detection(self, image: Image.Image) -> Dict[str, Any]:
        ...
        try:
            ...
            # Process results - assuming YOLO format
            # This could be extended to support different model output formats
            try:
                # Get the first result (only one image)
                result = results[0] if isinstance(results, list) else results

                # Convert YOLO results to our standard format
                boxes = result.boxes
                ...
                logger.warning(f"Error processing detection results: {str(e)}")
                # Just return empty detections if format is unexpected
                pass
        except Exception as e:
            logger.error(f"Error running object detection model: {str(e)}")
            detections = []
